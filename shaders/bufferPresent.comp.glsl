...
void main () {
	...
	// Sample the image and store the result
	 imageStore( image, ivec2( gl_GlobalInvocationID.xy ), ( distance( vec2( gl_GlobalInvocationID.xy ), GlobalData.mouseLoc ) < 100.0f ) ? vec4( 1.0f, 0.0f, 0.0f, 1.0f ) : texture( state, loc ) );
